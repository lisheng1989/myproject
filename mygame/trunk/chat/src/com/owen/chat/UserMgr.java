...
public class UserMgr {
	...
	private ConcurrentHashMap<Long,UserDB> userList = new ConcurrentHashMap<Long, UserDB>();
...
//	}
	...
	/**获得用户DB通过Userid*/
	public UserDB getUserDBByUserid(long userid)
	{
	...
	public UserDB createUserData(long userId,IoSession session) throws JSONException {
		UserDB uDB = null;
		if(userList.containsKey(userId))
		{
			uDB = userList.get(userId);
			uDB.isOnline = 1;
			uDB._session = session;
		}else{
			uDB = getUserDBBySql(userId);
			uDB._session = session;
			uDB.isOnline = 1;
		}
		if(uDB != null)
		{
			uDB._session.setAttribute("userId", userId);		
